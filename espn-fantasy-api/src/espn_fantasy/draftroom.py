...
against a live practice draft, rule out talking to it directly:
...
2. ESPN allows **one connection per team**. A second connection evicts the
   first with a "Duplicate Connection" notice. A headless client would kick
   you out of your own draft.
...
from espn_fantasy.names import normalize_player_key
...
() => {
  ...
  // The player currently up for bid. This is the moment bid guidance is worth
  // anything, so it is read every poll rather than inferred from sales.
...
def _money(raw: Any) -> int | None:
    ...
    ESPN renders an absent bid as the literal string `$null`, which is why
    this cannot simply strip non-digits and call `int` — that would turn "no
    bid" into a parse error and then, one `except` later, into a zero. Zero is
    a real price and would be believed.
    """
    if raw is None:
        return None
...
def parse_snapshot(raw: dict[str, Any]) -> RoomSnapshot:
    ...
    # The board is one flat grid of `team_count * slots` cells, laid out a
    # whole team at a time. Deriving the stride rather than hardcoding 15
    # keeps this correct for any roster size.
...
class DraftRoomReader:
    ...
    def connect(self) -> "DraftRoomReader":
        try:
            ...
            raise DraftRoomError(
                ...
                "  python -m pip install 'espn-fantasy-api[draftroom]'\n"
    ...
    def _find_page(self):
        ...
        With two draft rooms open — the real league beside yesterday's
        practice draft, or a stale tab left over from a reload — the tab that
        gets read is whichever Chrome happens to list first. That failure is
        invisible: the attach succeeds, a pre-flight check reports every team
        matched because both rooms belong to the same league, and the board
        simply never moves.

        So ambiguity is an error, never a coin flip. `tab_match` narrows it
        when you do want a specific one.
        """
        candidates = []
        for context in self._browser.contexts:
            for page in context.pages:
        ...
        if len(candidates) > 1:
            ...
            raise DraftRoomError(
                ...
                "never starts:\n"
                f"{listed}\n"
                "Close the ones you are not drafting in, or single one out "
                "with --tab <text from its URL>."
            )
        return candidates[0]
    ...
    def snapshot_raw(self) -> dict[str, Any]:
        ...
        Exposed so a caller wanting both the raw and the parsed view can
        derive them from *one* evaluate. Two evaluates would sample the board
        at two different instants, and a pick landing between them would look
        like a raw/parsed disagreement that is not real.
        ...
        raises on every call from another ("Current/Expected greenlet"). A
        polling loop on a worker thread therefore has to establish the
        connection *there*, which is what this lazy connect is for. Connecting
